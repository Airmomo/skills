...
description 编写的核心原则是**写"何时用"而非"是什么"**。Skill 路由本质是语义匹配——具体的触发场景比抽象的功能名词命中率更高。

❌ **错误示例（基于功能）：**

```yaml
description: 负责 PPT 生成。
description: SQL 生成器。
```
...
```yaml
description: 此 Skill 应在用户要求"制作季度业务汇报"、"准备融资路演 PPT"、"生成项目总结演示文稿"或提及基于幻灯片的汇报交付物时使用。
```

枚举**业务场景**（用户实际想完成什么），而不是功能动词。场景越具体，语义匹配得分越高。

#### 负样本描述原则（必需）

为了减少跨 Skill 误触发，必须明确说明 Skill **不适用**的场景。当存在相邻领域的兄弟 Skill 时尤其重要。

❌ **错误示例（无负向边界）：**

```yaml
description: 此 Skill 应在用户要求"生成 SQL"时使用。
```

✅ **正确示例（含负向边界）：**

```yaml
description: 此 Skill 应在用户要求"生成 SQL 查询"、"编写 SELECT 语句"时使用。不适用于数据库 Schema 设计、性能调优和迁移生成——这些应使用 db-schema-design 和 db-optimization Skill。
```

负样本有两个作用：一是锐化本 Skill 的边界，二是引导模型转向正确的兄弟 Skill。

#### 完整 description 模板

```yaml
description: 此 Skill 应在 [业务场景 1]、[业务场景 2] 或 [业务场景 3] 时使用。不适用于 [相邻领域 1] 或 [相邻领域 2]——那些应使用 [兄弟 Skill 名称]。
```
...
6. Skill 数量增长后出现误触发（用错 Skill）或漏触发（明明有却不用）
   **改进方式：** 三步组合拳——(1) 将 description 改为场景化描述；(2) 添加负样本边界；(3) 若 Skill 总数 > 15，启用分层组织，参考 `skill-organization.md`

## 大规模 Skill 组织

当 Plugin 的 Skill 数量从十几个增长到几十、上百个时，平铺式组织会导致路由失败——明明存在对应 Skill 却不被调用，或调用了错误的 Skill。这是因为 Skill 路由本质上是语义检索问题，搜索空间越大，命中率越低。

应对方案是三项配合使用：

1. **场景化 description**：上文已说明
2. **负样本描述**：上文已说明
3. **分层组织（Skill Tree）**：将 Skill 按业务领域/用户角色/任务类型分类，让模型先识别大类、再在大类内做精细匹配，缩小单次决策的搜索空间

完整的分类原则、三种 Skill Tree 实现方式（物理目录分层 / 索引文件 / description 前缀）、INDEX.md 模板和大规模案例对比，详见 **`skill-organization.md`**。
